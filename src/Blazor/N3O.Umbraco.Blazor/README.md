...
Hosts server-side Blazor inside an Umbraco site. `AddBlazor` registers the Blazor server services
and the composer maps the Blazor hub into the Umbraco pipeline, so components run without a separate
host.
