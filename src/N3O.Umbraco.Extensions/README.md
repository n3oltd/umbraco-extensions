...
class, whose `RegisterAll` scans the assemblies that `OurAssemblies` recognises as ours and
registers every type matching a predicate, so a package contributes behaviour by declaring a type
rather than by editing a registration list.
